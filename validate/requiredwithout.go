...
func RequiredWithout(input string, value any, payload map[string]any, options []string, sliceIndex string, errors map[string]interface{}, addError func(string, string, map[string]interface{}, string) map[string]interface{}, customeErrors map[string]string) (map[string]interface{}, bool) {
	...
	if validInput, exists := payload[options[0]]; !exists || helper.IsEmpty(validInput) {
		if _, exists_input := payload[input]; !exists_input {
			tmpError := fmt.Sprintf("El campo \"%s\" debe estar definido cuando el campo \"%s\" no está definido o esta vacio", input, options[0])

			if sliceIndex != "" {
				tmpError = fmt.Sprintf("El campo \"%s\" en la posición %s debe estar definido cuando el campo \"%s\" no está definido o esta vacio", input, sliceIndex, options[0])
			}
