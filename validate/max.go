...
func Max(input string, payload map[string]interface{}, options []string, errors map[string]interface{}, addError func(string, string, map[string]interface{}, string) map[string]interface{}) map[string]interface{} {
	...
	if err != nil {
		errors = addError(input, "max", errors, "El campo "+input+" debe ser un número")
		return errors
	}
	...
	if _, ok := value.(string); ok {
		...
		if strlen > int(max) {
			errors = addError(input, "max", errors, "El campo "+input+" debe tener como máximo "+options[0]+" caracteres")
		}
	}
